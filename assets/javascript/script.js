...
$(document).on('DOMContentLoaded', function () {
    if (document.querySelector('.ityped')) {
        window.ityped.init(document.querySelector('.ityped'), {
            strings: ['HI THERE!', 'I’M SANA AZEEZ_', 'B.Tech CS Student', 'Software & AI Enthusiast'],
            loop: true
        });
    }
});
...
// Function to check and trigger counter
function checkCounter() {
    if (counted === 0 && $('.counter').length) {
...
if (window.lenis) {
    window.lenis.on('scroll', checkCounter);
} else {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function toggleButton() {
        ...
        button.style.display = scrollTop > 100 ? 'block' : 'none';
    }
    
    // Use Lenis scroll event if available, otherwise use window scroll
    if (window.lenis) {
        window.lenis.on('scroll', toggleButton);
    } else {
